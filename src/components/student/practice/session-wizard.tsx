...
export function SessionWizard({
  ...
  // Validate title contains only alphanumeric characters and spaces
  const validateTitle = (titleValue: string): string => {
    if (!titleValue.trim()) {
      return "";
    }
    
    // Check if title contains only letters (A-Z, a-z), numbers (0-9), and spaces
    const alphanumericRegex = /^[A-Za-z0-9\s]+$/;
    if (!alphanumericRegex.test(titleValue)) {
      return "Title must contain only letters (A–Z) and numbers (0–9).";
    }
  ...
  const handleTitleChange = (value: string) => {
    ...
    setTitleError(error);
    setUserEditedTitle(true); // Mark as user-edited
  };
  ...
  const clearAllSelections = () => {
    ...
    setModuleIds([]);
    setCourseIds([]);
    setUserEditedTitle(false); // Reset user edit flag
    setTypes([]);
    setQuizYears([]);
    setQuizSourceIds([]);
    setQuestionCount(0);
    setTimeLimit(undefined);
    // Title will be auto-generated by useEffect when courseIds changes
  };
  ...
  const generateAlphabeticTitle = (): string => {
    const adjectives = [
      "Advanced", "Basic", "Clinical", "Diagnostic", "Essential", "Focused", "General", "Intensive",
      "Medical", "Practical", "Quick", "Review", "Specialized", "Targeted", "Comprehensive", "Interactive"
    ];
    const sessionTypes = [
      "Practice", "Session", "Study", "Review", "Training", "Preparation", "Assessment", "Evaluation",
      "Exercise", "Quiz", "Test", "Challenge", "Workout", "Drill"
    ];
    
    // Get course names for title generation
    ...
    // Add randomness with timestamp to ensure uniqueness
    const randomSeed = Date.now() + Math.random();
    const randomAdjective = adjectives[Math.floor(randomSeed * adjectives.length) % adjectives.length];
    const randomSessionType = sessionTypes[Math.floor(randomSeed * sessionTypes.length) % sessionTypes.length];
    ...
    // Fallback to adjective + session type with random number
    const randomNum = Math.floor(Math.random() * 100) + 1;
    return `${randomAdjective} ${randomSessionType} ${randomNum}`;
  };

  // Auto-generate title when wizard starts
  useEffect(() => {
    if (!title) {
      const autoTitle = generateAlphabeticTitle();
      setTitle(autoTitle);
      setTitleError(""); // Auto-generated titles are always valid
    }
  }, []);

  // Regenerate title when courses change (but only if user hasn't manually edited)
  useEffect(() => {
    if (!userEditedTitle) {
      const autoTitle = generateAlphabeticTitle();
      setTitle(autoTitle);
  ...
  // Step 1 validation: requires valid title (no error) and unit/module selection
  const step1Valid = !!title && !titleError && ((unitId && unitId !== "") || moduleIds.length > 0);
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={`quiz-input-enhanced w-full ${
                titleError ? "border-destructive focus:border-destructive focus:ring-destructive" : ""
              }`}
            ...
            {titleError && (
              <p 
                ...
                role="alert"
              >
                <span className="inline-block w-4 h-4 rounded-full bg-destructive/10 flex items-center justify-center flex-shrink-0">
            ...
            <p className="text-xs text-muted-foreground">
              Title is auto-generated and must contain only letters (A–Z) and numbers (0–9).
            </p>
          </div>
      ...
      {step === 3 && (
        <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="title-step3">Title</Label>
                <Input
                  id="title-step3"
                  ...
                  aria-invalid={!!titleError}
                  aria-describedby={titleError ? "title-error-step3" : undefined}
                />
                {titleError && (
                  <p 
                    id="title-error-step3" 
                    className="text-sm text-destructive mt-1 flex items-center gap-1"
                    role="alert"
                  >
                    <span className="inline-block w-4 h-4 rounded-full bg-destructive/10 flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          ...
          {step < 3 ? (
            ...
            <Button
              ...
              disabled={isCreating || !(step1Valid && step2Valid) || totalAvailable === 0 || questionCountLoading || !!questionCountError || sessionFiltersLoading || !!sessionFiltersError || !!titleError}
              className="practice-button w-full sm:w-auto"
              aria-disabled={isCreating || !(step1Valid && step2Valid) || totalAvailable === 0 || questionCountLoading || !!questionCountError || sessionFiltersLoading || !!sessionFiltersError || !!titleError}
            >
              {isCreating ? (
                <>
